def seriesIntoDict(df: pd.DataFrame) -> dict:
    ...
    series_dict = {row['name']: row['homepage'] + row['poster_path'] if row['homepage'] != 'NOT AVAILABLE' and row['poster_path'] != 'NOT AVAILABLE' else 'NOT AVAILABLE' for index, row in df.iterrows()}
